...
The Go module is `github.com/FukeKazki/issue-cli`; the binary (under `cmd/issue-cli/`) is `issue-cli`.

```sh
go build ./cmd/issue-cli          # build (output: ./issue-cli, gitignored)
go install ./cmd/issue-cli        # install as `issue-cli` on $PATH
go test ./...                     # all tests
...
- **`cmd/issue-cli/main.go`** — minimal arg parser, dispatches to `internal/cli`. Subcommands: `list`/`ls`, `new`, `edit`, `show`, `next`, `takt`, `help`. No flag library at the top level; subcommands use `flag.NewFlagSet`.
- **`internal/model`** — `Issue` struct + `Status` enum (`TODO` / `In Progress` / `Reviews` / `Done`). `OpenStatuses()` excludes `Done`. `ParseStatus` is the only canonical-strict string→`Status` path (used by `store.validate` to keep YAML on disk in canonical form); `ParseStatusFromCLI` is the case-insensitive/alias-tolerant variant used by `issue-cli edit --status`. `StatusRank` orders the enum and `(*Issue).AdvanceStatus(target)` is the forward-only setter used by auto-transitions (checkout) — prefer it over assigning `iss.Status` directly when the change is driven by an event rather than explicit user choice.
