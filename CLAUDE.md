...
- **`cmd/issue/main.go`** — minimal arg parser, dispatches to `internal/cli`. Subcommands: `list`/`ls`, `create`/`new`, `edit`, `_show` (internal), `help`. No flag library at the top level; subcommands use `flag.NewFlagSet`.
